...
export const PreviewFrame: React.FC<PreviewFrameProps> = ({ project, isLoading, onLog, logs }) => {
  ...
  const generateSrcDoc = (proj: WebProject) => {
    ...
    // Inject JS
    // Robust regex to remove the external script tag regardless of attributes
    finalHtml = finalHtml.replace(/<script\s+[^>]*src=["']script\.js["'][^>]*><\/script>/gi, '');
    
    // Safe inject script escaping closing tags to prevent early termination
